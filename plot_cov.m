function [] = plot_cov(folder, N, antenna_type, fc, ...
        ...
        for azimuthAngle = 1:angle_step:azimuthFinalAngle
            ...
            for elevationAngle = 1:angle_step:elevationFinalAngle
                ...
                for SNR_index = 1:SNR_len
                    ...
                    folder_new = strcat(folder, '/', int2str(SNR_list(SNR_index)), '/');
                    mkdir([pwd folder_new]);
                    
                    corrplot(realPart);
                    fileName = strcat(folder_new, int2str(azimuthAngle), '_', int2str(elevationAngle), '_real', '.png'); 
                    saveas(gcf, [pwd fileName]);
                    
                    corrplot(imagPart);
                    fileName = strcat(folder_new, int2str(azimuthAngle), '_', int2str(elevationAngle), '_imag', '.png'); 
                    saveas(gcf, [pwd fileName]);
                end
            end
        end    
