function [] = distance_label(name, N, antenna_type, PtdBm,  frequency, p_matrix, azimuthFinalAngle, elevationFinalAngle)
    ...
    for iter = 1:length(p_matrix)
        ...
        for azimuthAngle = 1:azimuthFinalAngle
            for elevationAngle = 1:elevationFinalAngle
                source_cordinates(azimuthAngle,elevationAngle,1) = p_rand*cosd(azimuthAngle)*cosd(elevationAngle); % x axis
                source_cordinates(azimuthAngle,elevationAngle,2) = p_rand*sind(azimuthAngle)*cosd(elevationAngle);% y axis
                source_cordinates(azimuthAngle,elevationAngle,3) = p_rand*sind(elevationAngle); % z axis
            end
        end
        ...
        Pr_normalized(1:azimuthFinalAngle,1:elevationFinalAngle,N+1)= p;
