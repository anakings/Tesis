...
def first_proposal():
	...
	accuracyOfModel_azimuth, time_validation_azimuth = modelObject.train_predict_regression(clf, name_model)
	print('Accuracy of azimuth model', name_model, 'is:', accuracyOfModel_azimuth)
	...
	accuracyOfModel_elevation, time_validation_elevation = modelObject.train_predict_regression(clf, name_model)
	print('Accuracy of elevation model', name_model, 'is:', accuracyOfModel_elevation)
	...
	return(listValues)
	'''
	print('\nAccuracy of 1st proposal with', name_model, 'is', accuracyOfModel_azimuth*accuracyOfModel_elevation/100)
	listValues = [d, accuracyOfModel_azimuth*accuracyOfModel_elevation/100, time_validation_azimuth + time_validation_elevation,accuracyOfModel_azimuth,accuracyOfModel_elevation]
	return(listValues)
...
if __name__ == '__main__':
	...
	Logs_Folder = './LogsRegression/'    #Folder where the results are saved
	...
	for phase in phase_list:
		...
		for distanceValue in distanceList:
			...
			for j in dirFiles:
				...
				open(Logs_Folder + str(phase) + '/'  + str(distanceValue) + '/' + str(j) + '/first_proposal/' + name_model + '.csv', 'w').close()
				#open(Logs_Folder + 'second_proposal/' + str(i) + '/' + modelName[name] + '.csv', 'w').close()
				open(Logs_Folder + str(phase) + '/' + str(distanceValue) + '/' + str(j) + '/third_proposal/' + name_model + '.csv', 'w').close()

				handleData = HandleData(phase, folder = Data_Folder + '/' + os.listdir(Data_Folder)[0] + '/' + str(j), one_hot_encode=False, phase_distance = phase_files, until=number_files, samples_by_distance=samples_distance) #folder = './Dround_Data_New/Nomalized'
				...
				for d in range(1, 11,2):
					...
					print(first_proposal_response)
					objectLogs.writeFile(Logs_Folder + str(phase) + '/' + str(distanceValue) + '/' + str(j) + '/first_proposal/' + name_model + '.csv', first_proposal_response)
					'''
